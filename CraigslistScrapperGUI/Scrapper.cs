...
namespace CraigslistScrapper
{
	public partial class frmScrapper : Form
	{
		...
		private void fillHowToSave()
		{
			Array lstEnums = Enum.GetValues(typeof(DataHandler.Enums.Enums.EnumDatabaseType));
			foreach (Enums.EnumDatabaseType enums in lstEnums)
			{
				Item itm = new Item(Enums.GetDescription(enums), enums.GetHashCode());
				ddlHowToSave.Items.Add(itm);
				if (itm.Value == 1)
				{
					ddlHowToSave.SelectedItem = itm;
				}
			}
		}
		...
		private void btnSearch_Click(object sender, EventArgs e)
		{
			...
			using (p)
			{
				...
				Item dbItem = (Item)ddlHowToSave.SelectedItem;
				string arguments = String.Format("{0} {1} {2} {3} {4} {5} {6} {7}",
					...
					(string.IsNullOrEmpty(dbItem.Value.ToString()) ? Enums.EnumDatabaseType.CSV.ToString() : dbItem.Value.ToString())
				);
